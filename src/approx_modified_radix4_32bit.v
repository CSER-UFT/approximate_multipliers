...
/**
 * Multiplicador Radix-4 Modificado com Codificador de Booth Aproximado
 * Combina a economia de energia da detecção de faixa (Modified Radix)
 * com a redução de hardware da aproximação de Booth (+2A/-2A -> +A/-A).
 */
module approx_modified_radix4_32bit (
    input  [31:0] a,
    input  [31:0] b,
    output [63:0] product
);
        ...
        for (i = 0; i < 16; i = i + 1) begin
            if (i <= range_b) begin
                case (extended_b[2*i +: 3])
                    ...
                    3'b011:         pp = (i < 4) ? (a_ext << (2*i)) : (a_ext << (2*i + 1));
                    3'b100:         pp = (i < 4) ? -(a_ext << (2*i)) : -(a_ext << (2*i + 1));
